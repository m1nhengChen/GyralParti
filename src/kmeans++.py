...
# For the original DeepWalk class (not used in clustering here)
...
# (Retained here for reference, though not used in the new clustering algorithm)
###############################################################################
class DeepWalk:
...
###############################################################################
# process_npz_file: Process a single .npz file using node features directly and
# local Lloyd algorithm + k-means++ (i.e. standard k-means with k-means++ initialization)
###############################################################################
def process_npz_file(npz_file, output_path, n_clusters, patience=50):
    """
    Process a single .npz file for clustering using the node features directly and
    applying k-means++ initialization with Lloyd’s algorithm.
    ...
    Returns:
        tuple: (overall conductance, overall modularity, overall mean NMI,
                LH time, RH time, total time)
    """
    ...
    # Clustering for LH partition using the features directly
    try:
        start = time.time()
        features_np_lh = features_torch_lh.cpu().numpy()
        # Use standard k-means with k-means++ initialization (Lloyd algorithm)
        kmeans = KMeans(n_clusters=n_clusters, init='k-means++', n_init=10, random_state=0)
        clusters = kmeans.fit_predict(features_np_lh)
        end = time.time()
        lh_time = end - start
        ...
        logging.error(f"Error during k-means++ clustering for LH partition in {npz_file}: {e}")
        return []
    ...
    # Clustering for RH partition using the features directly
    try:
        start = time.time()
        features_np_rh = features_torch_rh.cpu().numpy()
        kmeans = KMeans(n_clusters=n_clusters, init='k-means++', n_init=10, random_state=0)
        clusters = kmeans.fit_predict(features_np_rh)
        end = time.time()
        rh_time = end - start
        ...
        logging.error(f"Error during k-means++ clustering for RH partition in {rh_npz_file}: {e}")
        return []
...
def main():
    ...
    # List of cluster numbers to try (here using 20 as an example)
    cluster_numbers = [20]
